...
class AlignmentNet(nn.Module):
    ...
    def forward(self, X, Y):
        ...
        Y_embed = self.linear_Y(Y)
        return X_embed, Y_embed

    def fit(self, X, Y, num_iterations=500, learning_rate=0.001, lambda_l1=0.0001, device='cuda',test_size=0.2,fix_y=False):
        self.to(device)
        self.training_loss=[]
        self.validation_loss=[]
        self.model_weights=[]
        ...
        for _ in range(num_iterations):
            ...
            X_embed, Y_embed = self.forward(X_train, Y_train)
            ...
            # test set
            with torch.no_grad():
                X_test_embed, Y_test_embed = self.forward(X_test, Y_test)
    ...
    def score(self,X_test,Y_test,device='cuda'):
        ...
        with torch.no_grad():
            ...
            X_embed, Y_embed = self.forward(X_test, Y_test)
            loss = cost_func(X_embed, Y_embed)
            correlation=1-loss.cpu().detach().numpy()
...
if __name__ == '__main__':
    ...
    embedding_to_use=PCA(n_components=5).fit_transform(embedding_to_use)
    ...
    for j in range(n_repeat):
        ...
        for n_bin in range(len(X)):
            ...
            extractor.assign_Y_weights(np.eye(5),'cuda')
            ...
            extractor.fit(X_train_low_D, y_train, lambda_l1=0.0005, num_iterations=500,fix_y=True)
